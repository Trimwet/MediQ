-- Add Free plan to create_clinic RPC (patch for 20260823_create_clinic)
-- This updates the plan validation to include 'free'
...
BEGIN
  ...
  IF v_plan NOT IN ('free','starter','professional','enterprise') THEN
...
COMMENT ON FUNCTION public.create_clinic(text, text, text) IS
  ...
  'alphanumeric + hyphens, not reserved, unique), and plan (free/starter/professional/enterprise). '
